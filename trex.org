...
*** Data definition
  ...
  | ~nao_grid_num~      | ~dim~   |                                                      |                                                      | Total number of grid points for numerical orbitals                           |
  | ~interp_coeff_cnt~  | ~dim~   |                                                      |                                                      | Number of coefficients for the numerical orbital interpolator                |
  | ~nucleus_index~     | ~index~ | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | One-to-one correspondence between shells and atomic indices                  |
  | ~shell_ang_mom~     | ~int~   | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | One-to-one correspondence between shells and angular momenta                 |
  | ~shell_factor~      | ~float~ | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | Normalization factor of each shell ($\mathcal{N}_s$)                         |
  | ~r_power~           | ~int~   | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | Power to which $r$ is raised ($n_s$)                                         |
  | ~nao_grid_start~    | ~index~ | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | Index of the first data point for a given numerical orbital                  |
  | ~nao_grid_size~     | ~dim~   | ~[auxbasis.shell_num]~                               | ~(auxbasis.shell_num)~                               | Number of data points per numerical orbital                                  |
  | ~shell_index~       | ~index~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | One-to-one correspondence between primitives and shell index                 |
  | ~exponent~          | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Exponents of the primitives ($\gamma_{ks}$)                                  |
  | ~exponent_im~       | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Imaginary part of the exponents of the primitives ($\gamma_{ks}$)            |
  | ~coefficient~       | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Coefficients of the primitives ($a_{ks}$)                                    |
  | ~coefficient_im~    | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Imaginary part of the coefficients of the primitives ($a_{ks}$)              |
  | ~oscillation_arg~   | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Additional argument to have oscillating orbitals ($\beta_{ks}$)              |
  | ~oscillation_kind~  | ~str~   |                                                      |                                                      | Kind of Oscillating function:"Cos1" or "Cos2"                                |
  | ~prim_factor~       | ~float~ | ~[auxbasis.prim_num]~                                | ~(auxbasis.prim_num)~                                | Normalization coefficients for the primitives ($f_{ks}$)                     |
  | ~e_cut~             | ~float~ |                                                      |                                                      | Energy cut-off for plane-wave calculations                                   |
  | ~nao_grid_radius~   | ~float~ | ~[auxbasis.nao_grid_num]~                            | ~(auxbasis.nao_grid_num)~                            | Radii of grid points for numerical orbitals                                  |
  | ~nao_grid_phi~      | ~float~ | ~[auxbasis.nao_grid_num]~                            | ~(auxbasis.nao_grid_num)~                            | Wave function values for numerical orbitals                                  |
  | ~nao_grid_grad~     | ~float~ | ~[auxbasis.nao_grid_num]~                            | ~(auxbasis.nao_grid_num)~                            | Radial gradient of numerical orbitals                                        |
  | ~nao_grid_lap~      | ~float~ | ~[auxbasis.nao_grid_num]~                            | ~(auxbasis.nao_grid_num)~                            | Laplacian of numerical orbitals                                              |
  | ~interpolator_kind~ | ~str~   |                                                      |                                                      | Kind of spline, e.g. "Polynomial"                                            |
  | ~interpolator_phi~  | ~float~ | ~[auxbasis.nao_grid_num, auxbasis.interp_coeff_cnt]~ | ~(auxbasis.interp_coeff_cnt, auxbasis.nao_grid_num)~ | Coefficients for numerical orbital interpolation function                    |
  | ~interpolator_grad~ | ~float~ | ~[auxbasis.nao_grid_num, auxbasis.interp_coeff_cnt]~ | ~(auxbasis.interp_coeff_cnt, auxbasis.nao_grid_num)~ | Coefficients for numerical orbital gradient interpolation function           |
  | ~interpolator_lap~  | ~float~ | ~[auxbasis.nao_grid_num, auxbasis.interp_coeff_cnt]~ | ~(auxbasis.interp_coeff_cnt, auxbasis.nao_grid_num)~ | Coefficients for numerical orbital laplacian interpolation function          |

    #+CALL: json(data=auxbasis, title="auxbasis")
