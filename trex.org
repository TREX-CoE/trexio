...
* Hamiltonian
  ...
  The type of Hamiltonian can be:
  - ~Standard~: The non-relativistic electronic Hamiltonian
  - ~DFT~: Density functional theory (DFT)
  - ~RSDFT~: Range-separated DFT
    
  The description of the DFT Hamiltonian is still
  minimal. Contributions are welcome to develop that part.

** Range-separated potential

   The electron-electron Coulomb operator can be split into a short-range and
   a long-range potential ([[https://doi.org/10.1142/9789812830586_0004][Savin 1996]], [[https://doi.org/10.1103/PhysRevA.70.062505][Toulouse_2004]]):

   \[
   \frac{1}{r} = w_{\text{ee}}^{\text{sr}, \mu}(r) + w_{\text{ee}}^{\text{lr}, \mu}(r),
   \]
   where
   \[
   w_{\text{ee}}^{\text{sr},\mu}(r) = \frac{\text{erfc}(\mu\, r)}{r}
   \]
   and
   \[
   w_{\text{ee}}^{\text{lr},\mu}(r) = \frac{\text{erf}(\mu\, r)}{r}
   \]

   are the singular short-range (sr) part and the non-singular
   long-range (lr) part, respectively, $\mu$ is the range-separation
   parameter which controls how rapidly the short-range part decays,
   $\text{erf}(x)$ is the error function, and $\text{erfc}(x) = 1 - \erf(x)$ is its
...
** Parameters of the Hamiltonian
   ...
   | Variable | Type    | Dimensions | Description                                |
   |----------+---------+------------+--------------------------------------------|
   | ~type~   | ~str~   |            | Type of Hamiltonian (Standard, DFT, RSDFT) |
   | ~mu_rs~  | ~float~ |            | Range-separation parameter                 |
   
   #+CALL: json(data=hamiltonian, title="hamiltonian")
...
** Effective core potentials (ecp group)
   ...
   See [[http://dx.doi.org/10.1063/1.4984046][Trail 2017]] or [[https://doi.org/10.1063/1.5121006][Wang 2019]] for more info.
