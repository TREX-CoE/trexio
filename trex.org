...
*** Gaussian-type and Slater-type orbitals
  ...
  This factor is stored in the ~shell_factor~ field of the ~basis~ group (see the data definitions table below).
...
*** Flexibility and validation
  ...
  To verify that a TREXIO file is self-consistent, use the ~check-basis~
  and ~check-mo~ utilities provided by the [[https://github.com/TREX-CoE/trexio-tools][trexio-tools]] package.  These
  programs evaluate the AOs and MOs on a regular grid, compute a
  numerical approximation of the overlap matrices, and compare the
  result with the matrices stored in the TREXIO file.  Any
  inconsistency in the normalization factors will show up as a
  discrepancy between the numerical and stored overlaps.
...
# Note: the prim_factor values above normalize the full primitive basis functions
# (radial & angular) under the TREXIO regular solid harmonics convention.
    #+END_EXAMPLE
