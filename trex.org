...
**** Basis Functions
    ...
    The atomic basis set is defined as a list of shells. Each shell $s$
    is centered on a point $\mathbf{R}_A$, associated with a given
    angular momentum $\ell_s$, and characterized by a radial function
    $R_s(\mathbf{r})$. The radial function is a linear combination of
    ...
    **Slater-type orbitals (STO):**
    ...
    Slater orbitals are typically written as:

    \[
    R_s^{\text{STO}}(\mathbf{r}) = \mathcal{N}_s \left| \mathbf{r} - \mathbf{R}_A \right|^{n_s} 
    \sum_{k=1}^{N_{\text{prim}}} f_{ks}^{\text{STO}}(\gamma_{ks})\,a_{ks}\,
    \exp\left( -\gamma_{ks} \left| \mathbf{r} - \mathbf{R}_A \right| \right)
    \]

    For STOs, the prefactor exponent $n_s$ is usually set to 
    $n_s = n-1$, where $n$ is the principal quantum number.

    **Gaussian-type orbitals (GTO):**

    Gaussian orbitals are usually defined as:

    \[
    R_s^{\text{GTO}}(\mathbf{r}) = \mathcal{N}_s \left| \mathbf{r} - \mathbf{R}_A \right|^{n_s} 
    \sum_{k=1}^{N_{\text{prim}}} f_{ks}^{\text{GTO}}(\gamma_{ks})\, a_{ks}\, 
    \exp\left( -\gamma_{ks} \left| \mathbf{r} - \mathbf{R}_A \right|^2 \right)
    \]

    In conventional GTO-based codes, the radial prefactor exponent is
    typically set to \( n_s = 0 \), resulting in a so-called
    *cartesian Gaussian* formulation. However, it is also possible to
    define *spherically pure* Gaussian-type orbitals (GTOs) by setting
    \( n_s = \ell_s \).
...
**** Normalization Conventions
    ...
    constants should be explicitly provided in the data files to
    ensure portability and completeness.

    Some codes assume that the primitive functions are individually
    normalized. In that case, a normalization constant $f_{ks}$
    must be computed and applied to each primitive. If this
    normalization is not required, then $f_{ks} = 1$.
