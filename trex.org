...
** Atomic orbitals (ao group)
   ...
   The cartesian/spherical character can be defined for all the shells
   (either ~cartesian=0~ or ~1~, or it can be set on a per-shell basis using 
   the ~cartesian_shell~ array and ensuring that ~cartesian~ is not stored.

   #+NAME: ao
  | Variable          | Type    | Row-major Dimensions | Column-major Dimensions | Description                                                            |
  |-------------------+---------+----------------------+-------------------------+------------------------------------------------------------------------|
  | ~cartesian~       | ~int~   |                      |                         | ~0~: false, ~1~: true. If undefined, check the ~cartesian_shell~ array |
  ...
  | ~cartesian_shell~ | ~int~   | ~[basis.shell_num]~  | ~(basis.shell_num)~     | ~1~: the shell is cartesian, ~0~: the shell is spherical               |
   ...
   #+begin_src json :tangle trex.json
       "ao": {
         ...
         , "cartesian_shell" : [ "int"  , [ "basis.shell_num" ] ]
       } ,
   #+end_src
   :end:
