...
* Basis set (basis group)
  ...
  | ~shell_exp_n~   | ~int~   | ~(basis.shell_num)~ | One-to-one correspondence between shells and exponent for r (only for slater type orbitals) |
  ...
  #+begin_src python :tangle trex.json
	...
	,   "shell_exp_n" : [ "int"  , [ "basis.shell_num" ] ]
...
* Atomic orbitals (ao group)
  ...
  The ratial part \[R_{\theta(i)} (\mathbf{r})\] depends on
  the type of orbital, note that for Slater type orbitals,
  \[R_{\theta(i)} (\mathbf{r})=n^{n-l-1}e^{-\kappa(r-X)}\] and thus depends on the shell
  quantum number \[n\].
