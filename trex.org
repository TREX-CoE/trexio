...
* EPrivat tests (eprivat group)
  ...
  | Variable  | Type  | Row-major Dimensions | Column-major Dimensions | Description           |
  |-----------+-------+----------------------+-------------------------+-----------------------|
  | ~test~    | ~str~ |                      |                         | string test parameter |
  | ~numeric~ | ~int~ |                      |                         | string test parameter |

  #+CALL: json(data=eprivat, title="eprivat")
  #+RESULTS:
  :results:
  #+begin_src python :tangle trex.json
      "eprivat": {
...
*** Auxiliary basis set (auxbasis group, eprivat)
...
* SRAMPINOGROUP Auxiliary basis function
** Auxiliary basis set (auxbasis group)

  Duplicata of basis group for auxiliary basis function
