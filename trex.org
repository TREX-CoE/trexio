...
* Metadata (metadata group)
  ...
  **Note:** String attributes (type ~str~) have no hard-coded maximum
  length.  When writing an array of strings, the caller passes a
  ~max_str_len~ parameter that specifies the maximum number of
  characters (including the null terminator) for any single element.
  Strings longer than ~max_str_len - 1~ characters are rejected with
  ~TREXIO_INVALID_STR_LEN~.  In the Python API this parameter is
  computed automatically from the longest string in the list.
...
*** Data definitions
  ...
  | ~description~       | ~str~   |                                                |                                                | Optional description string for metadata, e.g. name, use, eetc               |
...
** Auxiliary basis sets (auxbasis groups)
...
*** Data definitions
    ...
    #+CALL: json(data=basis, title="auxbasis2", replace_group="basis")
	...
	"auxbasis2": {
	  ...
	  ,      "nao_grid_num" : [ "dim"  , []                                                 ]
	  ,  "interp_coeff_cnt" : [ "dim"  , []                                                 ]
	  ,     "nucleus_index" : [ "index", [ "auxbasis2.shell_num" ]                          ]
	  ,     "shell_ang_mom" : [ "int"  , [ "auxbasis2.shell_num" ]                          ]
	  ,      "shell_factor" : [ "float", [ "auxbasis2.shell_num" ]                          ]
	  ,           "r_power" : [ "int"  , [ "auxbasis2.shell_num" ]                          ]
	  ,    "nao_grid_start" : [ "index", [ "auxbasis2.shell_num" ]                          ]
	  ,     "nao_grid_size" : [ "dim"  , [ "auxbasis2.shell_num" ]                          ]
	  ,       "shell_index" : [ "index", [ "auxbasis2.prim_num" ]                           ]
	  ,          "exponent" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,       "exponent_im" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,       "coefficient" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,    "coefficient_im" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,   "oscillation_arg" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,  "oscillation_kind" : [ "str"  , []                                                 ]
	  ,       "prim_factor" : [ "float", [ "auxbasis2.prim_num" ]                           ]
	  ,             "e_cut" : [ "float", []                                                 ]
	  ,   "nao_grid_radius" : [ "float", [ "auxbasis2.nao_grid_num" ]                       ]
	  ,      "nao_grid_phi" : [ "float", [ "auxbasis2.nao_grid_num" ]                       ]
	  ,     "nao_grid_grad" : [ "float", [ "auxbasis2.nao_grid_num" ]                       ]
	  ,      "nao_grid_lap" : [ "float", [ "auxbasis2.nao_grid_num" ]                       ]
	  , "interpolator_kind" : [ "str"  , []                                                 ]
	  ,  "interpolator_phi" : [ "float", [ "auxbasis2.nao_grid_num", "auxbasis2.interp_coeff_cnt" ] ]
	  , "interpolator_grad" : [ "float", [ "auxbasis2.nao_grid_num", "auxbasis2.interp_coeff_cnt" ] ]
	  ,  "interpolator_lap" : [ "float", [ "auxbasis2.nao_grid_num", "auxbasis2.interp_coeff_cnt" ] ]
	} ,
    #+end_src
    :end:
...
** Auxiliary atomic orbitals (~auxao~ groups)

  Duplicate of the atomic orbitals group see documentation above.
  Used with the ~auxbasis~ groups to define auxiliary basis functions.
  ...
  #+CALL: json(data=ao, title="auxao2", replace_group="ao")
  ...
  #+begin_src json :tangle trex.json
      "auxao2": {
	      "cartesian" : [ "int"  , []           ]
	,           "num" : [ "dim"  , []           ]
	,         "shell" : [ "index", [ "auxao2.num" ] ]
	, "normalization" : [ "float", [ "auxao2.num" ] ]
      } ,
  #+end_src
  :end:
...
print("""#+end_src""")
 ...
 #+CALL: json(data=qmc, title="End", last=1)
