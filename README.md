...
Both interfaces are off by default and are built by their own tools, `dune` and
`cargo`, which the build systems invoke:

- `./configure --enable-ocaml` or `cmake -S. -Bbuild -DTREXIO_OCAML=ON`
- `./configure --enable-rust` or `cmake -S. -Bbuild -DTREXIO_RUST=ON`

Either way the binding is built against the library that was just compiled, not
against a copy installed on the system. The OCaml binding finds it with
pkg-config, for which a `trexio.pc` describing the build tree is generated; the
Rust one is handed the include and library directories, because a build tree is
not an installation prefix.

`make install` installs the OCaml library into the prefix, through `dune install`
(`make ocaml-opam-install` puts it in an opam switch instead). The Rust interface
has nothing to install: a crate is consumed from source, so `make check` and
`ctest` run `cargo test` and publishing the crate stays cargo's business.

Neither binding is regenerated by `make maintainer-clean`, and `dune` and `cargo`
keep their output inside `ocaml/trexio` and `rust/trexio` rather than in the
build directory, which is where those tools insist on putting it.
