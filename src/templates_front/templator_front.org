...
Prefixes in Fortran contain backend definitions.
...
* Coding conventions
  ...
  - API calls return ~trexio_exit_code~ (~int32_t~) whenever possible
...
** Back-ends
   ...
   The HDF5 file is binary and tailored to high-performance I/O. This backend is the default one.
...
** File opening
   ...
   `trexio_open` creates a new `trexio_t` file or opens existing one.
...
** File closing
   ...
   `trexio_close` closes a `trexio_t` file.

   `trexio_close` input:
   `file` -- (trexio_t) TREXIO file handle.
...
* Templates for front end
  ...
  Because of that, templates can be implemented and re-used. This approach
  minimizes the number of bugs as compared with manual copy-paste-modify scheme.

  All templates use the `$var$` notation to indicate the variable, which will
  be replaced by the `generator.py`. Sometimes the upper-case is used, i.e.
  `$VAR$`, which is useful, for example, in `#define` statements.
  More detailed description of each variable can be found below:
   ...
   | ~$group_dset_dtype$~          | 'Basic type of the dataset (int/float)'          | float            |
   | ~$group_dset_h5_dtype$~       | 'Type of the dataset in HDF5'                    | double           |
   | ~$group_dset_std_dtype_in$~   | 'Input type of the dataset in TEXT [C]'          | %lf              |
   | ~$group_dset_std_dtype_out$~  | 'Output type of the dataset in TEXT [C]'         | %24.16e          |
