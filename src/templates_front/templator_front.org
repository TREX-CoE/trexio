...
trexio_exit_code trexio_write_$group$_$group_dset$_32(trexio_t* const file, const $group_dset_dtype_single$* $group_dset$) {
  ...
  if ($group_dset$_64 == NULL) return TREXIO_ALLOCATION_FAILED;
  /* 
  TODO: implement a type conversion from single precision to double using conversion to str 
  May be an overkill for int arrays but definitely needed for floats
  See functions like snpring, gcvt etc.
  */
  /*
  for (uint64_t i=0; i<dim_size; ++i){
    TMP_VAR = FLOAT_TO_STR($group_dset$[i]);
    $group_dset$_64[i] = STR_TO_DOUBLE(TMP_VAR);
  }
  */
  switch (file->back_end) {
