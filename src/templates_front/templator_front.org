...
*** Introduction
   ...
   Strings are stored as UTF-8.  The ~max_str_len~ parameter refers to the
   number of *bytes* (as returned by C ~strlen~), not the number of Unicode
   characters.  The Python wrappers handle this automatically by encoding
   each string to UTF-8 before computing the length.
...
def write_$group_dset$(trexio_file, dset_w: list) -> None:
    ...
    max_str_length = len(max(dset_w, key=lambda s: len(s.encode('utf-8'))).encode('utf-8')) + 1
...
*** Introduction
   ...
   number of *bytes* (as returned by C ~strlen~), not the number of Unicode
   characters.  The Python wrappers handle this automatically by encoding
   the string to UTF-8 before computing the length.
...
def write_$group_str$(trexio_file, str_w: str) -> None:
    ...
    max_str_length = len(str_w.encode('utf-8')) + 1
