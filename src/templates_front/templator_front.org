...
,*/
  ...
  The file handle carries a mutex, so =trexio_mutex_t= below keeps
  =struct trexio_s= platform-independent. Windows has no pthreads: it maps
  to an =SRWLOCK=, which needs no teardown, so =TREXIO_MUTEX_DESTROY= is a
  no-op there. The lock is merely created with the handle and released with
  it, as no code path takes it yet. Both macros yield an =int= return code,
  zero on success, as =trexio_open= and =trexio_close= expect. They are
  macros and not =static inline= because =--enable-debug= combines
  =-Winline=, =-Werror= and =-fno-inline=, making any =inline= function fatal.
...
#ifdef _WIN32
#include <windows.h>
typedef SRWLOCK trexio_mutex_t;
#define TREXIO_MUTEX_INIT(mutex)    (InitializeSRWLock(mutex), 0)
#define TREXIO_MUTEX_DESTROY(mutex) ((void) (mutex), 0)
#else
#include <pthread.h>
typedef pthread_mutex_t trexio_mutex_t;
#define TREXIO_MUTEX_INIT(mutex)    pthread_mutex_init((mutex), NULL)
#define TREXIO_MUTEX_DESTROY(mutex) pthread_mutex_destroy(mutex)
#endif
  #+end_src
...
try:
    from . import pytrexio as pytr
except ImportError:
    raise Exception("Could not import the pytrexio extension module.")
...
*** Fortran
  ...
  integer(trexio_back_end_t), parameter :: TREXIO_MEMORY = 2
! integer(trexio_back_end_t), parameter :: TREXIO_JSON = 3
            ...
            const back_end_t back_end, trexio_exit_code* const rc_open)
{
  ...
  /* The in-memory back end is ephemeral: there is no on-disk file to reopen,
     so read-only mode is meaningless (and would otherwise fail later when the
     metadata is written). Reject it explicitly at open time. */
  if (back_end == TREXIO_MEMORY && mode == 'r') {
  ...
  case TREXIO_MEMORY:
    result_tmp = malloc(sizeof(trexio_memory_t));
    break;
...
,*/
  ...
  int irc = TREXIO_MUTEX_INIT( &(result->thread_lock) );
  if (irc != 0) {
...
,*/
  ...
  /* Write TREXIO_PACKAGE_VERSION upon creation of the file */
    ...
    case TREXIO_MEMORY:
      rc = trexio_memory_write_metadata_package_version(result, TREXIO_PACKAGE_VERSION);
      break;
  ...
  /* Otherwise read the metadata_package_version to get the TREXIO version upon creation of the file */
    ...
    case TREXIO_MEMORY:
      rc = trexio_memory_read_metadata_package_version(result, version_origin, 16);
      break;
  ...
  if (mode == 'u') {
    ...
    if (rc == TREXIO_HAS_NOT) {
      ...
      case TREXIO_MEMORY:
        rc = trexio_memory_write_metadata_unsafe(result, unsafe_val);
        break;
...
def _open(file_name: str, mode: str, back_end: int):
    ...
    try:
        ...
        # the C library already told us what went wrong (e.g. that the requested
        # back end is disabled), so do not mask it behind TREXIO_FILE_ERROR
        raise
    except:
...
,*/
  ...
  int irc = TREXIO_MUTEX_DESTROY( &(file->thread_lock) );
...
trexio_has_$group$ (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_has_$group$(file);
...
trexio_read_$group_num$_64 (trexio_t* const file, $group_num_dtype_double$* const num)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_num$(file, num);
    break;
...
trexio_write_$group_num$_64 (trexio_t* const file, const $group_num_dtype_double$ num)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_write_$group_num$(file, num_write);
...
trexio_read_$group_num$_32 (trexio_t* const file, $group_num_dtype_single$* const num)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_num$(file, &num_64);
    break;
...
trexio_write_$group_num$_32 (trexio_t* const file, const $group_num_dtype_single$ num)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_write_$group_num$(file, ($group_num_dtype_double$) num_write);
...
trexio_has_$group_num$ (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_has_$group_num$(file);
...
trexio_read_$group_dset$_64 (trexio_t* const file, $group_dset_dtype_double$* const $group_dset$)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_dset$(file, $group_dset$, rank, dims);
    break;
...
trexio_write_$group_dset$_64 (trexio_t* const file, const $group_dset_dtype_double$* $group_dset$)
{
  ...
  if ($is_index$) {
    ...
    case TREXIO_MEMORY:
      rc = trexio_memory_write_$group_dset$(file,
                                          (const $group_dset_dtype_double$*) $group_dset$_p,
                                          rank, dims);
      break;
    ...
    case TREXIO_MEMORY:
      rc = trexio_memory_write_$group_dset$(file, $group_dset$, rank, dims);
      break;
...
trexio_read_$group_dset$_32 (trexio_t* const file, $group_dset_dtype_single$* const $group_dset$)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_dset$(file, $group_dset$_64, rank, dims);
    break;
...
trexio_write_$group_dset$_32 (trexio_t* const file, const $group_dset_dtype_single$* $group_dset$)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_write_$group_dset$(file, $group_dset$_64, rank, dims);
    break;
...
trexio_has_$group_dset$ (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_has_$group_dset$(file);
...
#undef unique_rank
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_dset$(file, offset_file, *buffer_size, max_dim, &eof_read_size, index_sparse, value_sparse);
    break;
...
trexio_read_$group_dset$_size(trexio_t* const file, int64_t* const size_max)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_read_$group_dset$_size(file, size_max);
    break;
...
#undef unique_rank
  ...
  if (file->one_based) {
    ...
    case TREXIO_MEMORY:
      rc = trexio_memory_write_$group_dset$(file, offset_file, buffer_size, max_dim,
                                          size_max, index_sparse_p, value_sparse);
      break;
...
*** Introduction
   ...
   Only ASCII strings are supported, for compatibility with C and Fortran.
...
trexio_read_$group_dset$_low (trexio_t* const file, char* dset_out, const int32_t max_str_len)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_read_$group_dset$(file, dset_out, rank, dims, (uint32_t) max_str_len);
...
trexio_write_$group_dset$_low (trexio_t* const file, char* dset_in, const int32_t max_str_len)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_write_$group_dset$(file, (const char**) dset_str, rank, dims);
    break;
...
trexio_read_$group_dset$ (trexio_t* const file, const int64_t offset_file, int64_t* const buffer_size_read, double* const dset)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_$group_dset$(file, offset_file, rank, dims, &eof_read_size, dset);
    break;
...
trexio_write_$group_dset$ (trexio_t* const file, const int64_t offset_file, const int64_t buffer_size, const double* dset)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_write_$group_dset$(file, offset_file, rank, dims, dset);
    break;
...
*** Introduction
   ...
   Only ASCII strings are supported, for compatibility with C and Fortran.
   The Python wrappers enforce this by encoding strings as ASCII before
   writing; a ~UnicodeEncodeError~ is raised if non-ASCII characters are
   present.
...
trexio_read_$group_str$ (trexio_t* const file, char* const str_out, const int32_t max_str_len)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_read_$group_str$(file, str_out, (uint32_t) max_str_len);
...
trexio_write_$group_str$ (trexio_t* const file, const char* str, const int32_t max_str_len)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_write_$group_str$(file, str);
...
trexio_has_$group_str$ (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_has_$group_str$(file);
...
def write_$group_str$(trexio_file, str_w: str) -> None:
    ...
    max_str_length = len(str_w.encode('ascii')) + 1
...
trexio_delete_$group$ (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_delete_$group$(file);
...
trexio_read_determinant_list (trexio_t* const file, const int64_t offset_file, int64_t* const buffer_size_read, int64_t* const dset)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_read_determinant_list(file, offset_file, rank, dims, &eof_read_size, dset);
    break;
...
trexio_write_determinant_list (trexio_t* const file, const int64_t offset_file, const int64_t buffer_size, const int64_t* dset)
{
  ...
  case TREXIO_MEMORY:
    rc = trexio_memory_write_determinant_list(file, offset_file, rank, dims, dset);
    break;
...
trexio_has_determinant_list (trexio_t* const file)
{
  ...
  case TREXIO_MEMORY:
    return trexio_memory_has_determinant_list(file);
