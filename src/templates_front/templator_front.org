...
trexio_cp(const char* source, const char* dest)
{
  ...
  /* Check if source is a directory to use appropriate xcopy flags */
  if (back_end_local == TREXIO_TEXT) {
    /* TEXT backend uses directories - use directory copy flags */
    ret = snprintf(command, sizeof(command), "xcopy /E /I /Q /Y \"%s\" \"%s\"", source, dest);
  } else {
    /* HDF5 backend uses files - use file copy flags with /F to force file destination */
    ret = snprintf(command, sizeof(command), "xcopy /F /Q /Y \"%s\" \"%s\"", source, dest);
  }
