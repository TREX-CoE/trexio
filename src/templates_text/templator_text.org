...
trexio_text_init (trexio_t* const file)
{
  ...
  if (f->lock_file <= 0) {
#ifdef _WIN32
    /* On Windows, we can't reliably create lock files in all scenarios,
       so we just use a dummy file descriptor for compatibility */
    f->lock_file = 1; /* dummy file descriptor */
#else
    if (file->mode != 'r') {
      return TREXIO_ERRNO;
    } else {
...
#ifdef _WIN32
  /* On Windows, we use a dummy file descriptor, so don't close it */
  return TREXIO_SUCCESS;
#else
  struct flock fl;
