...
*/
  ...
  The text back end keeps each TREXIO file in a directory and probes it with
  the POSIX descriptor API, which the MSVC runtime spells with a leading
  underscore in =io.h= and =direct.h= rather than =unistd.h=, and which
  leaves =F_OK= undefined. The macros below supply that constant and route
  =access=, =open=, =close= and =mkdir= to whichever spelling exists;
  =TREXIO_MKDIR= drops the permission argument, as Windows directories carry
  no POSIX mode bits. They are named macros rather than bare =open=/=close=
  aliases because =trexio.c= includes =trexio_hdf5.h= after this header, and
  the HDF5 headers declare struct members of those very names.
...
#define F_OK 0
#endif
#define TREXIO_ACCESS(path, mode)       _access((path), (mode))
#define TREXIO_OPEN(path, flags, perms) _open((path), (flags), (perms))
#define TREXIO_CLOSE(fd)                _close(fd)
#define TREXIO_MKDIR(path, mode)        _mkdir(path)
#else
#include <unistd.h>
#define TREXIO_ACCESS(path, mode)       access((path), (mode))
#define TREXIO_OPEN(path, flags, perms) open((path), (flags), (perms))
#define TREXIO_CLOSE(fd)                close(fd)
#define TREXIO_MKDIR(path, mode)        mkdir((path), (mode))
#endif
#include <sys/stat.h>
#include <stdbool.h>
...
char* mkdtemp(char* template) {
    ...
    if (TREXIO_MKDIR(dir, S_IRWXU | S_IRWXG | S_IRWXO) != 0) {
        return NULL;
    }
...
trexio_text_init (trexio_t* const file)
{
  ...
  if (rc == TREXIO_FAILURE) {
    ...
    int rc_dir = TREXIO_MKDIR(file->file_name, 0777);
  ...
  f->lock_file = TREXIO_OPEN(file_name, O_WRONLY|O_CREAT|O_TRUNC, 0644);
    ...
    while(fscanf(f, "%1023s", buffer) != EOF) {
      ...
      // START REPEAT GROUP_DSET_STR
        ...
        if (size_$group_dset$ != 0) {
          ...
          /* First pass: measure the total length required to store all the
           ,* strings contiguously in a single block, instead of assuming an
           ,* arbitrary maximum string length which could truncate longer
           ,* strings and overflow the allocated block.
           ,*/
          long pos_$group_dset$ = ftell(f);
          size_t total_len_$group_dset$ = 0;
          for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
            ...
            total_len_$group_dset$ += strlen(buffer) + 1;
          }
          fseek(f, pos_$group_dset$, SEEK_SET);

          char* tmp_$group_dset$;
          tmp_$group_dset$ = CALLOC(total_len_$group_dset$, char);
          ...
          for (uint64_t i=0 ; i<size_$group_dset$ ; ++i) {
            ...
            size_t tmp_$group_dset$_len = strlen(buffer) + 1;
            memcpy(tmp_$group_dset$, buffer, tmp_$group_dset$_len);
            tmp_$group_dset$ += tmp_$group_dset$_len;
          }
        }
                                                ...
                                                const double* value_sparse)
{
  ...
  const char $group_dset$_file_name[] = "/$group_dset$.txt";
...
trexio_exit_code trexio_text_read_$group_dset$_size(trexio_t* const file, int64_t* const size_max)
{
  ...
  /* Build the name of the file with sparse data.
     The $group_dset$.txt is limited to 256 symbols for the moment. What are the chances that it will exceed?
   ,*/
  const char $group_dset$_file_name[] = "/$group_dset$.txt.size";
  /* The full path to the destination TXT file with sparse data. This will include TREXIO directory name. */
  char file_full_path[TREXIO_MAX_FILENAME_LENGTH];
...
trexio_exit_code trexio_text_has_$group_dset$(trexio_t* const file)
{
  ...
  if (TREXIO_ACCESS(file_full_path, F_OK) == 0){
    return TREXIO_SUCCESS;
  } else {
    return TREXIO_HAS_NOT;
  }
                                               ...
                                               double* const dset)
{
  ...
  const char file_name[] = "/$group_dset$.txt";
...
trexio_text_read_$group_dset$_size(trexio_t* const file, int64_t* const size_max)
{
  ...
  const char file_name[] = "/$group_dset$.txt.size";
                                                ...
                                                const double* dset)
{
  ...
  const char group_file_name[] = "/$group$.txt";
