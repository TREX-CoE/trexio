...
def delete_eprivat(trexio_file) -> None:
    """Delete the entire eprivat group from the TREXIO file.
    ...
    rc = pytr.trexio_delete_eprivat(trexio_file.pytrexio_s)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def has_eprivat_numeric(trexio_file) -> bool:
    """Check that eprivat_numeric variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_eprivat_numeric(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def has_eprivat_test(trexio_file) -> bool:
    """Check that eprivat_test variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_eprivat_test(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def has_eprivat(trexio_file) -> bool:
    """Check that eprivat group exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_eprivat(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def read_eprivat_numeric(trexio_file) -> int:
    """Read the eprivat_numeric variable from the TREXIO file.

    Parameter is a ~TREXIO File~ object that has been created by a call to ~open~ function.

    Returns:
        ~num_r~: int
        Integer value of eprivat_numeric variable read from ~trexio_file~.
    ...
    rc, num_r = pytr.trexio_read_eprivat_numeric(trexio_file.pytrexio_s)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def read_eprivat_test(trexio_file) -> str:
    """Read the eprivat_test variable from the TREXIO file.

    Parameter is a ~TREXIO File~ object that has been created by a call to ~open~ function.

    Returns:
        ~str_r~: str
        String corresponding to the eprivat_test variable read from ~trexio_file~.
    ...
    rc, str_r = pytr.trexio_read_eprivat_test(trexio_file.pytrexio_s, PYTREXIO_MAX_STR_LENGTH)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def write_eprivat_numeric(trexio_file, num_w: int) -> None:
    """Write the eprivat_numeric variable in the TREXIO file.
    ...
    num_w: int
        Value of the eprivat_numeric variable to be written.
    ...
    rc = pytr.trexio_write_eprivat_numeric(trexio_file.pytrexio_s, num_w)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def write_eprivat_test(trexio_file, str_w: str) -> None:
    """Write the eprivat_test variable in the TREXIO file.
    ...
    str_w: str
        String corresponding to the eprivat_test variable to be written.
    ...
    rc = pytr.trexio_write_eprivat_test(trexio_file.pytrexio_s, str_w, max_str_length)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
