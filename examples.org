...
Here is a simple example of a simple recipe to reorder your atomic orbitals in TREXIO
...
1. Specify the ordering used by your software in a list, for each angular momentum.
   Here is a simple example of an example for Molden ordering.
...
** Spherical AOs
  ...
  but you will need to define a function that generates a string so that
  the to ensure lexicographic sorting functions correctly.
